...
def bam_score_cha(input_bam_file_cha, output_bam_file_cha):
    ...
    realignment_ver11.write_reads(output_read_ls_cha, output_bam_file_cha, bam_header_cha)
...
def remove_mp(rm_input_bam_file, rm_output_bam_file, rm_mode, cores_num_rm, tmp_file_dic_rm):
    ...
    rm_bam_output_tmp_dic = tmp_file_dic_rm + '_' + ran_str + "_" + realignment_ver11.last_file_name(rm_input_bam_file) + 'rm_multi'

    os.mkdir(rm_bam_output_tmp_dic)

    # create tmp file
    mk_tmp_result = realignment_ver11.mk_temp_file(rm_input_bam_file, rm_bam_output_tmp_dic, 1000000)
    ...
    for ii in range(len(tmp_out_file_ls_rm)):
        bam_output_all_rm = realignment_ver11.bam_merge(tmp_out_file_ls_rm[ii], bam_output_all_rm)
...
if __name__ == "__main__":
    ...
    for i in range(len(sys.argv)):
        ...
        if input_para == '-i':
            ...
            print('-----------------------------------------------------Welcome to use multiple mapping removing-----------------------------------------------------')
            print('Usage:')
            print('    python remove_multi-mapping_ver2.py [Options]* {-i <bam_in>} [-o <bam_out>]')
            print('')
