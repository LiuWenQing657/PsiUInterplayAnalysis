...
#        print(cigar_change_result_ses)
        ...
        if cigar_change_result_ses[0]:
            ...
            out_read_ses.cigar = realignment_ver13.cigar_change(cigar_change_result_ses[2])  # cigar
...
def res_multi_proc(input_bam_rmp, output_bam_rmp, max_soft_clip_rmp, end_length_rmp, temp_dic_rmp, core_num_rmp):
    ...
    mk_temp_result = realignment_ver13.mk_temp_file(input_bam_rmp, temp_dic_rmp, 50000)
    ...
    for ii in range(len(temp_out_file_rmp)):
        bam_output_all_rmp = realignment_ver13.bam_merge(temp_out_file_rmp[ii], bam_output_all_rmp)

    bam_output_all_rmp.close()

    realignment_ver13.sort_bam_index(output_bam_file_temp_all_rmp, output_bam_rmp, core_num_rmp)
...
if __name__ == "__main__":
    ...
    for i in range(len(sys.argv)):
        ...
        if input_para == '-i':
            ...
            print('-----------------------------------------------------Welcome to use PBS-seq realignment-----------------------------------------------------')
            print('Usage:')
            print('    python remove_end_signal_ver2.py [Options]* {-i <bam_in>} [-o <bam_out>]')
            print('')
    ...
    temp_file_dic = temp_file_dic + '_' + ran_str + '_' + realignment_ver13.last_file_name(input_bam)
